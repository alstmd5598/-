...
async def on_ready():
    ...
    print("------------------")
    await client.change_presence(game=discord.Game(name='테스트중', type=1))
...
async def on_message(message):
    ...
    if message.content.startswith('!바보'):
        await client.send_message(message.channel, "혼날래요?")

    if message.content.startswith('!더기'):
        await client.send_message(message.channel, "멋있어요")

    if message.content.startswith('!멍청이'):
        await client.send_message(message.channel, "경고")

    if message.content.startswith('!잘가'):
        await client.send_message(message.channel, "안녕가세요^_^")

    if message.content.startswith('!지현이'):
        await client.send_message(message.channel, "공주님")

    if message.content.startswith('!밥먹었어'):
        await client.send_message(message.channel, "아녀 배고파염")



    if message.content.startswith('봇 제작자') or message.content.startswith('봇제작자'):
       embed = discord.Embed(title="봇 제작자", description="엠포짱#3533입니다.", color=0x00ff00)
...
async def on_message(message):
    if message.content.startswith("!정보"):
        ...
        embed.add_field(name="가입일", value=str(date.year) + "년" + ste(date.month) + "월" + str(date.day) + "일", inline=True)
    ...
    if message.content.startswith("!명령어"):
        ...
        embed.add_field(name='!솔로', value='!배그솔로 닉네임 형식으로 적으면 그 닉네임에대한 정보를 알려줍니다..', inline=False)
        embed.add_field(name='!듀오', value='!배그듀오 닉네임 형식으로 적으면 그 닉네임에대한 정보를 알려줍니다..', inline=False)
        embed.add_field(name='!스쿼드', value='!배그스쿼드 닉네임 형식으로 적으면 그 닉네임에대한 정보를 알려줍니다..', inline=False)
        ...
        embed.add_field(name='!정보', value='!정보 라고 적으면 본인에 프로필이  나옵니다.', inline=False)
...
client.run('NTgyNTAwNDM3MDA1OTU5MjAw.XOut7g.ciqjjaUNFNgprZ2lkoFoiFZ8uPI')
